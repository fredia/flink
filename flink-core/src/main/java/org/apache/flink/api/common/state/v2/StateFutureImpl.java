 ...
 * trigger, and most of the design are borrowed from the {@link
 * java.util.concurrent.CompletableFuture}. In the basic version of this implementation, we wrap
 * {@link java.util.concurrent.CompletableFuture} for simplification. TODO: remove
 * CompletableFuture.
 *
...
public class StateFutureImpl<T> implements InternalStateFuture<T> {
    ...
    public <U, V> StateFuture<V> thenCombine(
        ...
        try {
            if (completableFuture.isDone()) {
                ...
                StateFutureImpl<V> ret = new StateFutureImpl<>(mailboxEncapsulator);
    ...
    public <A> StateFutureImpl<A> makeNewStateFuture() {
        return new StateFutureImpl<>(mailboxEncapsulator);
    }
    ...
    public interface MailboxEncapsulator {
