...
/**
 * A priority queue with set semantics, implemented on top of ForSt. This uses a {@link TreeSet} to
 * cache the bytes of up to the first n elements from ForSt in memory to reduce interaction with
 * ForSt, in particular seek operations. Cache uses a simple write-through policy.
 *
 * @param <E> the type of the contained elements in the queue.
 */
public class ForStCachingPriorityQueueSet<E extends HeapPriorityQueueElement>
        extends AbstractHeapPriorityQueueElement implements InternalPriorityQueue<E> {

    /** Serialized empty value to insert into ForSt. */
    private static final byte[] DUMMY_BYTES = new byte[] {};

    /** The ForSt instance that serves as store. */
    @Nonnull private final RocksDB db;

    @Nonnull private final ReadOptions readOptions;

    /** Handle to the column family of the ForSt instance in which the elements are stored. */
    ...
    /** Wrapper to batch all writes to ForSt. */
    @Nonnull private final ForStWriteBatchWrapper batchWrapper;
    ...
    /** In memory cache that holds a head-subset of the elements stored in ForSt. */
    @Nonnull private final OrderedByteArraySetCache orderedCache;

    /**
     * This holds the key that we use to seek to the first element in ForSt, to improve
    ...
    /** This flag is true iff all elements in ForSt are also contained in the cache. */
    private boolean allElementsInCache;

    ForStCachingPriorityQueueSet(
            ...
            @Nonnull ForStWriteBatchWrapper batchWrapper,
    ...
    public boolean add(@Nonnull E toAdd) {
        ...
        if ((!cacheFull && allElementsInCache)
                || OrderedByteArraySetCache.LEXICOGRAPHIC_BYTE_COMPARATOR.compare(
                                toAddBytes, orderedCache.peekLast())
                        < 0) {
    ...
    private RocksBytesIterator orderedBytesIterator() {
        ...
        return new RocksBytesIterator(
                new ForStIteratorWrapper(db.newIterator(columnFamilyHandle, readOptions)));
    }

    /** Ensures that recent writes are flushed and reflect in the ForSt instance. */
    ...
    private void addToForSt(@Nonnull byte[] toAddBytes) {
        try {
    ...
    private void removeFromForSt(@Nonnull byte[] toRemoveBytes) {
        try {
    ...
    /**
     * Adapter between ForSt iterator and Java iterator. This is also closeable to release the
     * native resources after use.
     */
    private class RocksBytesIterator implements CloseableIterator<byte[]> {

        /** The ForSt iterator to which we forward ops. */
        @Nonnull private final ForStIteratorWrapper iterator;

        /** Cache for the current element of the iteration. */
        @Nullable private byte[] currentElement;

        private RocksBytesIterator(@Nonnull ForStIteratorWrapper iterator) {
