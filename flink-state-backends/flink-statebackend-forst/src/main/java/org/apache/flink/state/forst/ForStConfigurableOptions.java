...
/**
 * This class contains the configuration options for the {@link ForStStateBackend}.
 *
 * <p>Currently, ForSt's options would be configured by values here on top of {@link
 * PredefinedOptions}, and then a user-defined {@link ForStOptionsFactory} may override the
 * configurations here.
 */
@PublicEvolving
public class ForStConfigurableOptions implements Serializable {
    ...
    public static final ConfigOption<Boolean> INCREMENTAL_RESTORE_ASYNC_COMPACT_AFTER_RESCALE =
            key("state.backend.forst.incremental-restore-async-compact-after-rescale")
                    .booleanType()
                    .defaultValue(Boolean.FALSE)
                    .withDescription(
                            "If true, an async compaction of ForSt is started after every restore after which we detect keys (including tombstones) in the database that are outside the key-groups range of the backend.");
