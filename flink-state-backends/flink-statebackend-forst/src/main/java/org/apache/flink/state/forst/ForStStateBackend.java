...
import static org.apache.flink.state.forst.ForStConfigurableOptions.INCREMENTAL_RESTORE_ASYNC_COMPACT_AFTER_RESCALE;
 ...
 * ForSt} instance. This state backend can store very large state that exceeds memory and spills to
 * local disk. All key/value state (including windows) is stored in the key/value index of ForSt.
 * For persistence against loss of machines, please configure a {@link
...
public class ForStStateBackend extends AbstractManagedMemoryStateBackend
    ...
    /**
     * The configuration for ForSt priorityQueue state settings (priorityQueue state type, etc.).
     */
    ...
    /**
     * Max consumed memory size for one batch in {@link ForStWriteBatchWrapper}, default value 2mb.
     */
    ...
    /**
     * Whether we use the optimized Ingest/Clip DB method for rescaling ForSt incremental
     * checkpoints.
     */
    ...
    /**
     * Whether to leverage deleteFilesInRange API to clean up useless ForSt files during rescaling.
     */
    private final TernaryBoolean rescalingUseDeleteFilesInRange;

    /** Factory for Write Buffer Manager and Block Cache. */
    private ForStMemoryFactory forStMemoryFactory;
    // ------------------------------------------------------------------------

    /** Creates a new {@code ForStStateBackend} for storing local state. */
     ...
     */
    public ForStStateBackend(boolean enableIncrementalCheckpointing) {
     ...
     * @param enableIncrementalCheckpointing True if incremental checkpointing is enabled.
     */
    public ForStStateBackend(TernaryBoolean enableIncrementalCheckpointing) {
    ...
    private ForStStateBackend(
        ...
        // configure ForSt predefined options
        this.predefinedOptions =
                original.predefinedOptions == null
                        ? PredefinedOptions.valueOf(config.get(ForStOptions.PREDEFINED_OPTIONS))
    ...
    /**
     * Gets the memory configuration object, which offers settings to control ForSt's memory usage.
     */
    public ForStMemoryConfiguration getMemoryConfiguration() {
        return memoryConfiguration;
    }

    /**
     * Sets the path where the ForSt local database files should be stored on the local file system.
     * Setting this path overrides the default behavior, where the files are stored across the
     * configured temp directories.
     *
    ...
    /**
     * Sets the directories in which the local ForSt database puts its files (like SST and metadata
     ...
     *
     * <p>Under these directories on the TaskManager, ForSt stores its SST files and metadata files.
     * These directories do not need to be persistent, they can be ephermeral, meaning that they are
    ...
    /**
     * Sets the predefined options for ForSt.
     *
     * <p>If user-configured options within {@link ForStConfigurableOptions} is set (through
     * config.yaml) or a user-defined options factory is set (via {@link
     * #setForStOptions(ForStOptionsFactory)}), then the options from the factory are applied on top
     * of the here specified predefined options and customized options.
     *
    ...
    /**
     * Gets the currently set predefined options for ForSt. The default options (if nothing was set
     * via {@link #setPredefinedOptions(PredefinedOptions)}) are {@link PredefinedOptions#DEFAULT}.
     *
     * <p>If user-configured options within {@link ForStConfigurableOptions} is set (through
     * config.yaml) of a user-defined options factory is set (via {@link
     * #setForStOptions(ForStOptionsFactory)}), then the options from the factory are applied on top
     * of the predefined and customized options.
     *
     * @return The currently set predefined options for ForSt.
     */
    @VisibleForTesting
     ...
     * in {@link ForStConfigurableOptions}.
     *
    ...
    public void setNumberOfTransferThreads(int numberOfTransferThreads) {
        Preconditions.checkArgument(
                ...
                "The number of threads used to transfer files in ForStStateBackend should be greater than zero.");
        this.numberOfTransferThreads = numberOfTransferThreads;
    }

    /** Gets the max batch size will be used in {@link ForStWriteBatchWrapper}. */
    ...
    /**
     * Sets the max batch size will be used in {@link ForStWriteBatchWrapper}, no positive value
     * will disable memory size controller, just use item count controller.
     *
     * @param writeBatchSize The size will used to be used in {@link ForStWriteBatchWrapper}.
     */
    ...
    public enum PriorityQueueStateType implements DescribedEnum {
        ...
        FORST(text("Implementation based on ForSt"));
