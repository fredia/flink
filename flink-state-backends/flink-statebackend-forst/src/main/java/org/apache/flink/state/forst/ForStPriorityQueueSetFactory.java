 ...
 * for ForSt backend.
 */
public class ForStPriorityQueueSetFactory implements PriorityQueueSetFactory {
    ...
    private final Map<String, ForStKeyedStateBackend.ForStKvStateInfo> kvStateInformation;
    ...
    public <T extends HeapPriorityQueueElement & PriorityComparable<? super T> & Keyed<?>>
        ...
        final ForStKeyedStateBackend.ForStKvStateInfo stateCFHandle =
        ...
        return new KeyGroupPartitionedPriorityQueue<>(
                ...
                new KeyGroupPartitionedPriorityQueue.PartitionQueueSetFactory<
                        T, ForStCachingPriorityQueueSet<T>>() {
                    @Nonnull
                    @Override
                    public ForStCachingPriorityQueueSet<T> create(
                            int keyGroupId,
                            int numKeyGroups,
                            @Nonnull KeyExtractorFunction<T> keyExtractor,
                            @Nonnull PriorityComparator<T> elementPriorityComparator) {
                        TreeOrderedSetCache orderedSetCache = new TreeOrderedSetCache(cacheSize);
                        return new ForStCachingPriorityQueueSet<>(
                                keyGroupId,
                                keyGroupPrefixBytes,
                                db,
                                readOptions,
    ...
    @Nonnull
    private <T> ForStKeyedStateBackend.ForStKvStateInfo tryRegisterPriorityQueueMetaInfo(
            @Nonnull String stateName,
            @Nonnull TypeSerializer<T> byteOrderedElementSerializer,
            boolean allowFutureMetadataUpdates) {

        ForStKeyedStateBackend.ForStKvStateInfo stateInfo = kvStateInformation.get(stateName);
        ...
        if (stateInfo == null) {
            ...
            if (previousElementSerializer != byteOrderedElementSerializer) {
                ...
                // Since priority queue elements are written into ForSt
                ...
                stateInfo =
                        new ForStKeyedStateBackend.ForStKvStateInfo(
