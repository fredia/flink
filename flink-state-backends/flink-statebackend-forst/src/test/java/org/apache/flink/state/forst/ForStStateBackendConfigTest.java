...
public class ForStStateBackendConfigTest {
    ...
    public void testSetDbPath() throws Exception {
        ...
        assertNull(forStStateBackend.getDbStoragePaths());

        forStStateBackend.setDbStoragePath(testDir1);
        assertArrayEquals(new String[] {testDir1}, forStStateBackend.getDbStoragePaths());

        forStStateBackend.setDbStoragePath(null);
        assertNull(forStStateBackend.getDbStoragePaths());

        forStStateBackend.setDbStoragePaths(testDir1, testDir2);
        assertArrayEquals(new String[] {testDir1, testDir2}, forStStateBackend.getDbStoragePaths());
        ...
        try {
            ...
            forStStateBackend.setDbStoragePaths(null);
            assertNull(forStStateBackend.getDbStoragePaths());
        } finally {
    ...
    public void testConfigureTimerService() throws Exception {
        ...
        Assert.assertEquals(
                ...
                ForStOptions.TIMER_SERVICE_FACTORY.key());

        // Fix the option value string and ensure all are covered
        Assert.assertEquals(2, ForStStateBackend.PriorityQueueStateType.values().length);
        Assert.assertEquals("FORST", ForStStateBackend.PriorityQueueStateType.FORST.toString());
        Assert.assertEquals("HEAP", ForStStateBackend.PriorityQueueStateType.HEAP.toString());

        // Fix the default
        Assert.assertEquals(
                ForStStateBackend.PriorityQueueStateType.FORST,
                ForStOptions.TIMER_SERVICE_FACTORY.defaultValue());
        ...
        ForStKeyedStateBackend<Integer> keyedBackend =
                createKeyedStateBackend(forStStateBackend, env, IntSerializer.INSTANCE);
        Assert.assertEquals(
                ForStPriorityQueueSetFactory.class,
                keyedBackend.getPriorityQueueFactory().getClass());
        keyedBackend.dispose();

        Configuration conf = new Configuration();
        conf.set(ForStOptions.TIMER_SERVICE_FACTORY, ForStStateBackend.PriorityQueueStateType.HEAP);

        forStStateBackend =
                forStStateBackend.configure(conf, Thread.currentThread().getContextClassLoader());
        keyedBackend = createKeyedStateBackend(forStStateBackend, env, IntSerializer.INSTANCE);
        Assert.assertEquals(
    ...
    public void testConfigureRocksDBPriorityQueueFactoryCacheSize() throws Exception {
        final MockEnvironment env = getMockEnvironment(tempFolder.newFolder());
        ForStStateBackend rocksDbBackend = new ForStStateBackend();
        int cacheSize = 512;
        Configuration conf = new Configuration();
        conf.set(
                ForStOptions.TIMER_SERVICE_FACTORY, ForStStateBackend.PriorityQueueStateType.FORST);
        conf.set(ForStOptions.FORST_TIMER_SERVICE_FACTORY_CACHE_SIZE, cacheSize);
        ...
        Assert.assertEquals(
                cacheSize,
                ((ForStPriorityQueueSetFactory) keyedBackend.getPriorityQueueFactory())
                        .getCacheSize());
        keyedBackend.dispose();
        env.close();
    }
    ...
    public void testConfigureTimerServiceLoadingFromApplication() throws Exception {
        ...
        final ForStStateBackend backend = new ForStStateBackend();
        backend.setPriorityQueueStateType(ForStStateBackend.PriorityQueueStateType.HEAP);
        ...
        configFromConfFile.setString(
                ForStOptions.TIMER_SERVICE_FACTORY.key(),
                ForStStateBackend.PriorityQueueStateType.FORST.toString());

        // configure final backend from job and cluster config
        final ForStStateBackend configuredRocksDBStateBackend =
                backend.configure(
    ...
    private void testLocalDbPaths(String configuredPath, File expectedPath) throws Exception {
        final ForStStateBackend rocksDbBackend = new ForStStateBackend();
    ...
    public void testSetEmptyPaths() throws Exception {
        ForStStateBackend forStStateBackend = new ForStStateBackend();
        forStStateBackend.setDbStoragePaths();
    }
    ...
    public void testNonFileSchemePath() throws Exception {
        ForStStateBackend forStStateBackend = new ForStStateBackend();
        forStStateBackend.setDbStoragePath("hdfs:///some/path/to/perdition");
    }

    @Test(expected = IllegalArgumentException.class)
    public void testDbPathRelativePaths() throws Exception {
        ForStStateBackend forStStateBackend = new ForStStateBackend();
        forStStateBackend.setDbStoragePath("relative/path");
    }
    ...
    public void testCleanRelocatedDbLogs() throws Exception {
        ...
        File instanceRocksDBPath =
                ForStKeyedStateBackendBuilder.getInstanceForStPath(instanceBasePath);
    ...
    public void testUseTempDirectories() throws Exception {
        ...
        ForStKeyedStateBackend<Integer> keyedBackend =
                (ForStKeyedStateBackend<Integer>)
    ...
    public void testFailWhenNoLocalStorageDir() throws Exception {
        ...
        try (MockEnvironment env = getMockEnvironment(tempFolder.newFolder())) {
            forStStateBackend.setDbStoragePath(targetDir.getAbsolutePath());
    ...
    public void testPredefinedOptions() throws Exception {
        ...
        assertEquals(PredefinedOptions.DEFAULT, forStStateBackend.getPredefinedOptions());

        // verify that user could configure predefined options via config.yaml
        Configuration configuration = new Configuration();
        configuration.set(
                ForStOptions.PREDEFINED_OPTIONS, PredefinedOptions.FLASH_SSD_OPTIMIZED.name());
        forStStateBackend = new ForStStateBackend();
        forStStateBackend = forStStateBackend.configure(configuration, getClass().getClassLoader());
        assertEquals(
                PredefinedOptions.FLASH_SSD_OPTIMIZED, forStStateBackend.getPredefinedOptions());

        // verify that predefined options could be set programmatically and override pre-configured
        // one.
        forStStateBackend.setPredefinedOptions(PredefinedOptions.SPINNING_DISK_OPTIMIZED);
        assertEquals(
                PredefinedOptions.SPINNING_DISK_OPTIMIZED,
                forStStateBackend.getPredefinedOptions());
    }
    ...
    public void testConfigurableOptionsFromConfig() throws Exception {
        ...
        // verify illegal configuration
        {
            ...
            verifyIllegalArgument(ForStConfigurableOptions.RESTORE_OVERLAP_FRACTION_THRESHOLD, "2");
        }
        ...
        // verify legal configuration
        {
            ...
            configuration.setString(
                    ForStConfigurableOptions.RESTORE_OVERLAP_FRACTION_THRESHOLD.key(), "0.5");
    ...
    public void testDefaultRestoreOverlapThreshold() {
        ForStStateBackend rocksDBStateBackend = new ForStStateBackend(true);
        assertTrue(
                ForStConfigurableOptions.RESTORE_OVERLAP_FRACTION_THRESHOLD.defaultValue()
    ...
    public void testConfigureRestoreOverlapThreshold() {
        ForStStateBackend rocksDBStateBackend = new ForStStateBackend(true);
        Configuration configuration = new Configuration();
        configuration.set(ForStConfigurableOptions.RESTORE_OVERLAP_FRACTION_THRESHOLD, 0.3);
        rocksDBStateBackend =
    ...
    public void testDefaultUseIngestDB() {
        ForStStateBackend rocksDBStateBackend = new ForStStateBackend(true);
        assertEquals(
                ForStConfigurableOptions.USE_INGEST_DB_RESTORE_MODE.defaultValue(),
    ...
    public void testConfigureUseIngestDB() {
        ForStStateBackend rocksDBStateBackend = new ForStStateBackend(true);
        Configuration configuration = new Configuration();
        configuration.set(ForStConfigurableOptions.USE_INGEST_DB_RESTORE_MODE, true);
        rocksDBStateBackend =
                rocksDBStateBackend.configure(configuration, getClass().getClassLoader());
    ...
    public void testDefaultUseDeleteFilesInRange() {
        ForStStateBackend rocksDBStateBackend = new ForStStateBackend(true);
        assertEquals(
                ForStConfigurableOptions.USE_DELETE_FILES_IN_RANGE_DURING_RESCALING.defaultValue(),
    ...
    public void testConfigureUseFilesInRange() {
        ...
        Configuration configuration = new Configuration();
        configuration.set(
                ForStConfigurableOptions.USE_DELETE_FILES_IN_RANGE_DURING_RESCALING,
                !ForStConfigurableOptions.USE_DELETE_FILES_IN_RANGE_DURING_RESCALING
                        .defaultValue());
        rocksDBStateBackend =
                rocksDBStateBackend.configure(configuration, getClass().getClassLoader());
    ...
    public void testDefaultIncrementalRestoreInstanceBufferSize() {
        ForStStateBackend rocksDBStateBackend = new ForStStateBackend(true);
        assertEquals(
                ForStConfigurableOptions.INCREMENTAL_RESTORE_ASYNC_COMPACT_AFTER_RESCALE
                        .defaultValue(),
