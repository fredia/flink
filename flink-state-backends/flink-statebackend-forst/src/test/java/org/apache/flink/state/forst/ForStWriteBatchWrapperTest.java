...
/** Tests to guard {@link ForStWriteBatchWrapper}. */
public class ForStWriteBatchWrapperTest {
    ...
    /**
     * Tests that {@link ForStWriteBatchWrapper} flushes after the memory consumed exceeds the
     * preconfigured value.
     */
    @Test
    public void testWriteBatchWrapperFlushAfterMemorySizeExceed() throws Exception {
        try (RocksDB db = RocksDB.open(folder.newFolder().getAbsolutePath());
                ...
                ColumnFamilyHandle handle =
                        db.createColumnFamily(new ColumnFamilyDescriptor("test".getBytes()));
                ForStWriteBatchWrapper writeBatchWrapper =
                        new ForStWriteBatchWrapper(db, options, 200, 50)) {
            ...
            // more information please ref write_batch.cc in ForSt
    ...
    /**
     * Tests that {@link ForStWriteBatchWrapper} flushes after the kv count exceeds the
     * preconfigured value.
     */
    @Test
    public void testWriteBatchWrapperFlushAfterCountExceed() throws Exception {
        try (RocksDB db = RocksDB.open(folder.newFolder().getAbsolutePath());
                ...
                ColumnFamilyHandle handle =
                        db.createColumnFamily(new ColumnFamilyDescriptor("test".getBytes()));
                ForStWriteBatchWrapper writeBatchWrapper =
                        new ForStWriteBatchWrapper(db, options, 100, 50000)) {
            long initBatchSize = writeBatchWrapper.getDataSize();
    ...
    /**
     * Test that {@link ForStWriteBatchWrapper} creates default {@link WriteOptions} with disabled
    ...
    public void testDefaultWriteOptionsHaveDisabledWAL() throws Exception {
        ...
        try (RocksDB db = RocksDB.open(folder.newFolder().getAbsolutePath());
                ForStWriteBatchWrapper writeBatchWrapper =
                        new ForStWriteBatchWrapper(db, null, 200, 50)) {
    ...
    /**
     * Test that {@link ForStWriteBatchWrapper} respects passed in {@link WriteOptions} and does not
     * close them.
     */
    @Test
    public void testNotClosingPassedInWriteOption() throws Exception {
        try (WriteOptions passInOption = new WriteOptions().setDisableWAL(false)) {
            try (RocksDB db = RocksDB.open(folder.newFolder().getAbsolutePath());
                    ForStWriteBatchWrapper writeBatchWrapper =
                            new ForStWriteBatchWrapper(db, passInOption, 200, 50)) {
